...
const teacherLabel = (usr) => {
  const key = (usr?.username || "").toLowerCase();
  if (key === "admin") return "admin";
  if (key === "nauczyciel1" || key === "piotr") return "nauczyciel 1";
  if (key === "nauczyciel2" || key === "monika") return "nauczyciel 2";
...
export default function App() {
  ...
  const logout = async (options = {}) => {
    const { remainingSeconds = null, saveProgress = true } = options || {};
    if (user?.sid && saveProgress) await api.logout(user.sid, remainingSeconds);
...
function PrivacyModal({ onClose }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(2,8,23,0.9)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div style={{ ...S.card, maxWidth: 520, width: "100%", maxHeight: "80vh", overflowY: "auto", padding: 24 }} onClick={e => e.stopPropagation()}>
        <h2 style={{ margin: "0 0 14px", fontSize: 18, color: "#38bdf8" }}>🔒 Polityka prywatności</h2>
        <div style={{ color: "#94a3b8", fontSize: 13, lineHeight: 1.7 }}>
          <p>MathClass <b>nie zbiera danych osobowych uczniów</b>. Logowanie odbywa się przy użyciu numeru w dzienniku oraz nazwy klasy (np. <code style={{ background: "#1e293b", padding: "1px 5px", borderRadius: 4 }}>05_1A</code>), bez imienia ani nazwiska.</p>
          <p>Powiązanie numeru w dzienniku z konkretnym uczniem przechowuje wyłącznie nauczyciel — aplikacja nie ma do tego dostępu i tych danych nie przetwarza.</p>
          <p>System zapisuje wyłącznie: numer konta, klasę, udzielone odpowiedzi na zadania, pozostały czas sesji oraz status blokady konta.</p>
          <p>Hasła (PIN-y) przechowywane są w formie zaszyfrowanej (bcrypt) i nie da się ich odczytać.</p>
        </div>
        <Bt onClick={onClose} style={{ marginTop: 20 }}>Zamknij</Bt>
      </div>
    </div>
  );
}
...
function Login({ onLogin }) {
  ...
  const goStudent = async () => {
    if (!num.trim() || !classId || !pin.trim()) { setErr("Wypełnij numer, klasę i PIN."); return; }
    const cr = cls.find(c => c.id === classId);
    if (!cr) { setErr("Wybierz klasę."); return; }
    const username = `${num.trim().padStart(2, "0")}_${cr.name}`;
    setLd(true); setErr("");
    const { data, error } = await api.login(username, pin.trim());
    if (error || !data) { setErr(error?.error || "Nieprawidłowy numer, klasa lub PIN."); setLd(false); return; }
    localStorage.setItem("mc_token", data.token);
    onLogin(data.user);
    setLd(false);
  };
      ...
      <div style={{ width: "100%", maxWidth: 420, padding: "0 16px" }}>
        ...
        <div style={{ ...S.card, padding: "24px 20px" }}>
          <div style={{ display: "flex", marginBottom: 20, background: "#0f172a", borderRadius: 8, padding: 3 }}>
            {[["student", "Uczeń"], ["teacher", "Nauczyciel"]].map(([k, l]) => (
              <button key={k} onClick={() => { setMode(k); setErr(""); }}
                style={{ flex: 1, padding: "8px", border: "none", borderRadius: 6, cursor: "pointer", fontWeight: 600, fontSize: 13, background: mode === k ? "#1e3a5f" : "transparent", color: mode === k ? "#38bdf8" : "#64748b" }}>{l}</button>
            ))}
          </div>

          {mode === "student" ? (<>
            <Lb>Numer w dzienniku</Lb>
            <In type="number" v={num} s={setNum} ph="np. 5" oe={goStudent} />
            <Lb style={{ marginTop: 14 }}>Klasa</Lb>
            <select value={classId} onChange={e => setClassId(e.target.value)} style={S.inp}>
              <option value="">Wybierz klasę…</option>
              {cls.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
            </select>
            <Lb style={{ marginTop: 14 }}>PIN</Lb>
            <In type="password" v={pin} s={v => setPin(v.replace(/\D/g, "").slice(0, 4))} ph="••••" oe={goStudent} />
            {err && <Eb>{err}</Eb>}
            <Bt onClick={goStudent} loading={ld} style={{ marginTop: 20 }}>Zaloguj się →</Bt>
          </>) : (<>
            <Lb>Login</Lb>
            <In v={u} s={setU} ph="login" oe={goTeacher} />
            <Lb style={{ marginTop: 14 }}>Hasło</Lb>
            <In type="password" v={p} s={setP} ph="••••••" oe={goTeacher} />
            {err && <Eb>{err}</Eb>}
            <Bt onClick={goTeacher} loading={ld} style={{ marginTop: 20 }}>Zaloguj się →</Bt>
          </>)}
        </div>
        <p style={{ textAlign: "center", marginTop: 16 }}>
          <button onClick={() => setShowPrivacy(true)} style={{ background: "none", border: "none", color: "#475569", fontSize: 11, cursor: "pointer", textDecoration: "underline" }}>
...
function Student({ user, onLogout }) {
  ...
  useEffect(() => {
    const loadTasks = async () => {
      ...
      const [{ data: tasksData }, { data: answersData }] = await Promise.all([
        api.get("tasks", { active: "true" }),
        api.get("answers", { user_id: user.id, limit: 1000 }),
      ]);
      if (!tasksData) return;
      const solvedIds = new Set((answersData || []).filter(a => a.correct).map(a => a.task_id));
      const filtered = (allowedCats ? tasksData.filter(t => allowedCats.has(t.category)) : tasksData).filter(t => !solvedIds.has(t.id));
      const shuffled = [...filtered].sort(() => Math.random() - .5);
      setSlots(shuffled.slice(0, 6));
  ...
  const sub = async (idx) => {
    ...
    if (isOk) {
      setFb(p => ({ ...p, [t.id]: "ok" }));
      const nextRemaining = Math.max(0, remainingCount - 1);
      ...
      setTimeout(async () => {
        ...
        const solvedIds = new Set((answersData || []).filter(a => a.correct).map(a => a.task_id));
        const pool2 = (tasksData || []).filter(x => !ids.includes(x.id) && !solvedIds.has(x.id) && (!allowedCats || allowedCats.has(x.category)));
        const next = pool2.length ? pool2[Math.floor(Math.random() * pool2.length)] : null;
        setSlots(p => { const s = [...p]; s[idx] = next ?? null; return s; });
  ...
  useEffect(() => {
    if (!loaded || finished || remainingCount !== 0 || !slots.length) return;
    const finish = async () => {
      if (user.sid) {
        await api.patch("users", user.id, { session_locked: true, remaining_seconds: 0 }).catch(() => {});
        await api.patch("sessions", user.sid, { active: false, locked_at: new Date().toISOString() }).catch(() => {});
      }
      setTimeout(() => onLogout({ saveProgress: false }), 900);
    };
    const timer = setTimeout(finish, 300);
    return () => clearTimeout(timer);
  }, [loaded, remainingCount, finished, slots.length, user.id, user.sid, onLogout]);

  if (finished || (!exp && loaded && remainingCount === 0 && slots.length > 0)) {
    return (
      <div style={S.cx}>
        <div style={{ textAlign: "center", background: "#0f172a", borderRadius: 20, padding: "40px 24px", border: "1px solid #22c55e", maxWidth: 420, margin: "0 16px" }}>
          <div style={{ fontSize: 56 }}>🎉</div>
          <h2 style={{ color: "#22c55e", fontSize: 20 }}>Ukończyłeś wszystkie zadania</h2>
          <p style={{ color: "#64748b", fontSize: 14, marginBottom: 8 }}>Wyniki zostały wysłane do nauczyciela.</p>
          <p style={{ color: "#475569", fontSize: 12, marginBottom: 0 }}>Konto zostanie teraz zablokowane i nastąpi wylogowanie.</p>
        </div>
      </div>
    );
  }
  if (exp) return (
    <div style={S.cx}>
      <div style={{ textAlign: "center", background: "#0f172a", borderRadius: 20, padding: "40px 24px", border: "1px solid #ef4444", maxWidth: 360, margin: "0 16px" }}>
        ...
        <Bt onClick={() => onLogout({ saveProgress: false })} style={{ marginTop: 24 }}>Wróć do logowania</Bt>
      </div>
    </div>
  );
    ...
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "16px 12px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <button onClick={() => onLogout({ remainingSeconds: tl })} style={{ ...S.gh, fontSize: 12, padding: "6px 12px" }}>Wyloguj</button>
        </div>
      </div>
...
function Admin({ user, onLogout }) {
  ...
  const tabs = [["s","👥 Uczniowie"],["t","📝 Zadania"],["st","📊 Statystyki"],["as","➕ Uczeń"],["gen","🎲 Generuj klasę"],["at","➕ Zadanie"],["cl","🏫 Klasy"],["cat","🏷️ Kategorie"],["assign","📋 Przydziały"],["bug","🐛 Zgłoś błąd"]];
  return (
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ color: "#475569", fontSize: 12, marginTop: 3 }}>
            Zalogowany: <b style={{ color: "#94a3b8" }}>{teacherLabel(user)}</b> · MathClass
...
function StatTab({ teacherCls }) {
  ...
  const exportAll = async () => {
    ...
    for (const s of visible) {
      ...
      (ans || []).forEach(h => rows.push({ "Imię": s.name, "Klasa": s.class || "–", "Login": s.username, "Kategoria": h.category || "–", "Pytanie": h.question || h.task_id, "Poprawna": h.correct_answer || "–", "Odpowiedź": h.given_answer, "Wynik": h.correct ? "TAK" : "NIE", "Data": new Date(h.answered_at).toLocaleString("pl-PL") }));
    }
    if (!rows.length) { alert("Brak danych."); return; }
...
// ── GenClass — generator kont z numerem w dzienniku ──────────────
function GenClass({ teacherCls }) {
  const [cls, setCls] = useState([]);
  const [classId, setClassId] = useState("");
  ...
  useEffect(() => {
    api.get("classes").then(({ data }) => setCls((data || []).filter(c => !teacherCls || teacherCls.has(c.id))));
  }, []);

  const generate = async () => {
    if (!classId || !count) { setErr("Wybierz klasę i podaj liczbę uczniów."); return; }
    const cr = cls.find(c => c.id === classId);
    if (!cr) return;
    setErr(""); setLd(true);
    const { data, error } = await api.post("users/generate-class", {
      class_id: classId,
      class_name: cr.name,
      count: parseInt(count),
      session_minutes: mins ? parseInt(mins) : null,
    });
    setLd(false);
    if (error) { setErr("Nie udało się dodać ucznia. Sprawdź dane i spróbuj ponownie."); return; }
    setResult({ className: cr.name, accounts: data.generated });
  };
  ...
  const exportCsv = () => {
    ...
    const rows = result.accounts.map(a => ({
      "Login": a.username,
      "PIN": a.pin || "(już istniał)",
      "Klasa": result.className,
    }));
    exportToExcel(rows, `konta_${result.className}_${new Date().toLocaleDateString("pl-PL").replace(/\./g, "-")}`);
  };

  if (result) {
    return (
      <div style={{ maxWidth: 560 }}>
        <Ok style={{ marginBottom: 14 }}>✅ Wygenerowano konta dla klasy {result.className}</Ok>
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          <button onClick={exportCsv} style={{ padding: "8px 16px", borderRadius: 8, border: "1px solid #16a34a", background: "#052e16", color: "#4ade80", fontSize: 13, cursor: "pointer", fontWeight: 600 }}>📥 Eksportuj CSV (do wydruku)</button>
          <button onClick={() => setResult(null)} style={{ ...S.gh, fontSize: 13 }}>← Generuj kolejną klasę</button>
        </div>
        <div style={{ ...S.card, padding: 0, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "#1e293b" }}>
                <th style={{ padding: "10px 14px", textAlign: "left", fontSize: 12, color: "#94a3b8" }}>Login</th>
                <th style={{ padding: "10px 14px", textAlign: "left", fontSize: 12, color: "#94a3b8" }}>PIN</th>
              </tr>
            </thead>
            <tbody>
              {result.accounts.map(a => (
                <tr key={a.username} style={{ borderTop: "1px solid #1e293b" }}>
                  <td style={{ padding: "9px 14px", fontSize: 13, fontFamily: "monospace" }}>{a.username}</td>
                  <td style={{ padding: "9px 14px", fontSize: 13, fontFamily: "monospace", color: a.pin ? "#22c55e" : "#ef4444" }}>{a.pin || "już istniało"}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
        <p style={{ color: "#475569", fontSize: 11, marginTop: 10 }}>⚠️ Zapisz lub wydrukuj tę listę — PIN-y nie będą ponownie widoczne po opuszczeniu tego ekranu.</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ ...S.card, maxWidth: 480, padding: 24 }}>
      <h2 style={{ margin: "0 0 6px", fontSize: 17 }}>🎲 Generuj konta klasy</h2>
      <p style={{ color: "#64748b", fontSize: 12, margin: "0 0 18px" }}>Tworzy konta w formacie <code style={{ background: "#1e293b", padding: "2px 5px", borderRadius: 4 }}>01_1A</code>...<code style={{ background: "#1e293b", padding: "2px 5px", borderRadius: 4 }}>NN_1A</code> z losowym 4-cyfrowym PIN-em. Numery są nadawane kolejno.</p>
      <Lb>Klasa</Lb>
      <select value={classId} onChange={e => setClassId(e.target.value)} style={S.inp}>
        <option value="">Wybierz klasę…</option>
        {cls.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
      </select>
      <Lb style={{ marginTop: 12 }}>Liczba uczniów w klasie</Lb>
      <In type="number" v={count} s={setCount} ph="np. 28" />
      <Lb style={{ marginTop: 12 }}>Czas sesji (min)</Lb>
      <In type="number" v={mins} s={setMins} ph="puste = bez limitu" />
      {err && <Eb>{err}</Eb>}
      <Bt onClick={generate} loading={ld} style={{ marginTop: 18 }}>🎲 Generuj konta</Bt>
    </div>
  );
}
// ── AddStud ──────────────────────────────────────────────────────
function AddStud({ teacherCls, onAdded }) {
...
// ── BugReportTab — zgłaszanie błędów przez nauczyciela ───────────
function BugReportTab({ user }) {
  const [msg, setMsg] = useState("");
  ...
  const send = async () => {
    if (!msg.trim()) { setErr("Wpisz treść zgłoszenia."); return; }
    setErr(""); setLd(true);
    const { error } = await api.post("bug-reports", {
      reporter_username: user.username,
      reporter_role: user.role,
      message: msg.trim(),
    });
    setLd(false);
    if (error) { setErr("Nie udało się wysłać zgłoszenia. Spróbuj ponownie."); return; }
    setSent(true); setMsg("");
  };

  if (sent) {
    return (
      <div style={{ ...S.card, maxWidth: 480, padding: 24, textAlign: "center" }}>
        <div style={{ fontSize: 40 }}>✅</div>
        <h2 style={{ margin: "10px 0 6px", fontSize: 17 }}>Zgłoszenie wysłane</h2>
        <p style={{ color: "#64748b", fontSize: 13, marginBottom: 18 }}>Dziękujemy, administrator otrzyma zgłoszenie.</p>
        <Bt onClick={() => setSent(false)} style={{ background: "#1e293b" }}>Wyślij kolejne</Bt>
      </div>
    );
  }
  ...
  return (
    <div style={{ ...S.card, maxWidth: 480, padding: 24 }}>
      <h2 style={{ margin: "0 0 6px", fontSize: 17 }}>🐛 Zgłoś problem</h2>
      <p style={{ color: "#64748b", fontSize: 12, margin: "0 0 16px" }}>Opisz napotkany problem — błąd techniczny, literówkę w zadaniu, itp.</p>
      <Lb>Opis problemu</Lb>
      <textarea value={msg} onChange={e => setMsg(e.target.value)} placeholder="Co się stało? Co próbowałeś/aś zrobić?" style={{ ...S.inp, height: 120, resize: "vertical" }} />
      {err && <Eb>{err}</Eb>}
      <Bt onClick={send} loading={ld} style={{ marginTop: 16 }}>📤 Wyślij zgłoszenie</Bt>
    </div>
  );
}
...
function SuperAdmin({ user, onLogout }) {
  ...
  const tabs = [["teachers","👩‍🏫 Nauczyciele"],["classes","🏫 Klasy"],["students","👥 Uczniowie"],["bugs","🐛 Zgłoszenia"],["db","🗄️ Baza danych"]];
  return (
    <div style={{ maxWidth: 960, margin: "0 auto", padding: "16px 12px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
      ...
      {tab === "students" && <div style={{ display: "grid", gap: 14 }}><AddStud teacherCls={null} onAdded={() => setStudentRefresh(v => v + 1)} /><SAStudentsTab refreshKey={studentRefresh} /></div>}
      {tab === "bugs"     && <BugsAdminTab />}
      {tab === "db"       && <DbTab />}
    </div>
  );
}
...
function SAStudentsTab({ refreshKey = 0 }) {
  ...
  const toggleLock = async u => {
    ...
    await api.patch("users", u.id, { session_locked: nl, remaining_seconds: nl ? u.remaining_seconds || null : null });
  ...
  const deleteStudent = async u => {
    if (!confirm(`Usunąć ucznia ${u.name || u.username}?`)) return;
    await api.del("category-assignments", null, { target_type: "user", target_id: u.id });
    await api.del("answers", null, { user_id: u.id });
    await api.del("users", u.id);
    setList(p => p.filter(x => x.id !== u.id));
    sm("🗑️ Usunięto ucznia.");
  };
        ...
        {visible.map(u => (
          <div key={u.id} style={{ ...S.card, display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", padding: "12px 14px", borderLeft: `3px solid ${sc(u)}` }}>
            ...
            <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
              ...
              <Sm onClick={() => deleteStudent(u)} col="#1c0505">🗑️</Sm>
            </div>
          </div>
        ))}
        {visible.length === 0 && <Em>Brak uczniów.</Em>}
          ...
          return (
            <div key={usr.id} style={{ ...S.card, border: isExpanded ? "1px solid #0369a1" : "1px solid #1e293b" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", flexWrap: "wrap" }}>
                <div style={{ flex: 1 }}>
                    <span style={{ fontWeight: 700, fontSize: 14 }}>{teacherLabel(usr)}</span>
...
// ── BugsAdminTab — lista zgłoszeń dla superadmina ─────────────────
function BugsAdminTab() {
  const [list, setList] = useState([]);
  ...
  const load = () => api.get("bug-reports", fil !== "all" ? { status: fil } : {}).then(({ data }) => { setList(data || []); setLd(false); });
  useEffect(() => { setLd(true); load(); }, [fil]);

  const resolve = async (id) => {
    await api.patch("bug-reports", id, { status: "resolved" });
    setList(p => p.filter(x => x.id !== id || fil === "all"));
    sm("✅ Oznaczono jako rozwiązane.");
  };

  const reopen = async (id) => {
    await api.patch("bug-reports", id, { status: "open" });
    load();
    sm("🔄 Przywrócono jako otwarte.");
  };

  const rm = async (id) => {
    if (!confirm("Usunąć zgłoszenie na stałe?")) return;
    await api.del("bug-reports", id);
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
        {[["open","🔴 Otwarte"],["resolved","✅ Rozwiązane"],["all","Wszystkie"]].map(([k,l]) => (
          <Pi key={k} a={fil === k} onClick={() => setFil(k)}>{l}</Pi>
        ))}
      </div>
      <div style={{ display: "grid", gap: 8 }}>
        {list.map(b => (
          <div key={b.id} style={{ ...S.card, padding: "14px 16px", borderLeft: `3px solid ${b.status === "open" ? "#f59e0b" : "#22c55e"}` }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10, marginBottom: 8 }}>
              <div>
                <span style={{ fontWeight: 700, fontSize: 13 }}>{b.reporter_username || "nieznany"}</span>
                <span style={{ marginLeft: 6, fontSize: 10, background: "#1e293b", padding: "2px 7px", borderRadius: 20, color: "#7dd3fc" }}>{b.reporter_role === "admin" ? "nauczyciel" : b.reporter_role}</span>
              </div>
              <span style={{ fontSize: 10, color: "#475569" }}>{new Date(b.created_at).toLocaleString("pl-PL")}</span>
            </div>
            <p style={{ margin: "0 0 10px", fontSize: 13, lineHeight: 1.5, color: "#cbd5e1", whiteSpace: "pre-wrap" }}>{b.message}</p>
            <div style={{ display: "flex", gap: 6 }}>
              {b.status === "open"
                ? <Sm onClick={() => resolve(b.id)} col="#052e16">✅ Rozwiąż</Sm>
                : <Sm onClick={() => reopen(b.id)} col="#1c1200">🔄 Otwórz ponownie</Sm>}
              <Sm onClick={() => rm(b.id)} col="#1c0505">🗑️ Usuń</Sm>
            </div>
          </div>
        ))}
        {!list.length && <Em>Brak zgłoszeń.</Em>}
      </div>
    </div>
  );
}
...
function In({ v, s, ph, type = "text", disabled, oe, style = {} }) {
  return <input type={type} value={v} disabled={disabled} placeholder={ph} autoComplete="off" onChange={e => s(e.target.value)} onKeyDown={e => e.key === "Enter" && oe && oe()} style={{ ...S.inp, ...style }} />;
}
...
const S = {
  root: { minHeight: "100vh", background: "#020817", fontFamily: "'Segoe UI',system-ui,sans-serif", color: "#e2e8f0", position: "relative", paddingBottom: 96 },
