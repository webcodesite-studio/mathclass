...
cd backend
cp .env.example .env
# Uzupełnij .env: DB_PASSWORD, JWT_SECRET, DISCORD_WEBHOOK_URL
npm install
npm run dev
# API działa na http://localhost:3001
```

`DISCORD_WEBHOOK_URL` jest opcjonalny, ale jeśli go ustawisz, nowe zgłoszenia błędów będą wysyłane na wskazany kanał Discord.
...
| `nauczyciel1` | `nauczyciel1` | nauczyciel  |
| `nauczyciel2` | `nauczyciel2` | nauczyciel  |
